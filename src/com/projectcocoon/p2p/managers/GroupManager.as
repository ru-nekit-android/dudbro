package com.projectcocoon.p2p.managers
{
	...
	public class GroupManager extends EventDispatcher
	{
		...
		public function createNetGroup(name:String):NetGroup
		{
			...
			var groupInfo:GroupInfo = new GroupInfo(groupSpecString, name);
		...
		{
			return sendMessageToAll(null, netGroup, CommandType.SERVICE, CommandList.ANNOUNCE_CLIENT);
		}
		...
		public function getGroupName(netGroup:NetGroup):String
		{
			var groupInfo:GroupInfo = groups[netGroup];
			if (groupInfo)
			{
		...
		private function handlePosting(event:NetStatusEvent):void
		{
			...
			{
				if (message.command == CommandList.ANNOUNCE_CLIENT) 
				{
					for each (var client:ClientVO in groupInfo.clients) 
					{
						if(client.groupID == message.client.groupID) 
						{
							client.uid			= message.client.uid;
							client.type			= message.client.type;
							client.clientName = message.client.clientName;
							dispatchEvent(new ClientEvent(ClientEvent.CLIENT_ANNONCE, client, group));
							break;
						}
					}
				}
...
class GroupInfo
{
	...
	public function GroupInfo(groupSpec:String, name:String)
	{
